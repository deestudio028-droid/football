"""Phase 29: Operational Live Prospective Collection Execution Script.

Executes the first genuine operational collection cycle for the frozen V4.6 Physical Draw Gate candidate.

INVARIANTS:
1. Zero Historical Contamination: Excludes historical 1,301 diagnostic & Phase 25 450 cohorts.
2. Strict Pre-Kickoff Timing: Enforces prediction_timestamp <= scheduled_kickoff - 15 minutes.
3. Cryptographic SHA-256 Lock: All prediction payloads are canonicalized and hashed.
4. Two-Stage Separation: Zero outcome recording during collection.
5. Idempotent: Can be executed repeatedly without duplicate rows.
"""
...
logger = logging.getLogger("run_phase29_live_collection")

MATCHES_DB = PROJECT_ROOT / "data" / "processed" / "matches.db"
FEATURES_DB = PROJECT_ROOT / "data" / "processed" / "features.db"
V4_ARTIFACT_PATH = PROJECT_ROOT / "data" / "models" / "v4_poisson_venue_elo_online_ad.pkl"
MANDATORY_POWER_TARGET = 1050


def run_phase29_collection(
    ...
    """Execute Phase 29 operational live prospective collection cycle."""
    ...
    provider_class_name = fixture_provider.__class__.__name__
    ...
    try:
        ...
        summary = {
            ...
            "remaining_to_power_gate": max(0, MANDATORY_POWER_TARGET - len(locked_ids)),
            "dry_run": dry_run,
            "status": "NO LIVE PROVIDER CONFIGURED (WAITING SAFELY FOR CREDENTIALS/FEED)",
        }
        _print_phase29_summary(summary)
        return summary
    ...
    # Load Model Artifacts & Pre-Match Features
    ...
    for f in upcoming_fixtures:
        ...
        # Historical / Phase 25 check
        if fid in historical_ids:
            historical_cnt += 1
            continue

        # Kickoff boundary check
        ...
        # Pre-Match Probability & Gate Evaluation
        ...
        # Cryptographically Lock Prediction
    ...
    status_str = "DRY_RUN_COMPLETED" if dry_run else "COLLECTION_CYCLE_COMPLETE"
...
def _print_phase29_summary(summary: Dict[str, Any]) -> None:
    print("=" * 88)
    print("PHASE 29: TRUE LIVE PROSPECTIVE COLLECTION EXECUTION")
    print("=" * 88)
    print(f"Timestamp:                 {summary['timestamp']}")
    print(f"Provider Source:           {summary['provider']}")
    ...
    print(f"Historical IDs Rejected:   {summary['historical_rejected']}")
    print(f"Late Kickoff Rejected:     {summary['late_fixtures_rejected']}")
    ...
    print(f"Remaining to N=1,050 Gate: {summary['remaining_to_power_gate']}")
    print(f"Dry Run Mode:              {summary['dry_run']}")
    print(f"Cycle Status:              {summary['status']}")
    print("=" * 88)


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Phase 29 Live Prospective Collection Execution")
    ...
    parser.add_argument("--provider", type=str, default="default", choices=["default", "oddalerts", "local"], help="Fixture provider")
