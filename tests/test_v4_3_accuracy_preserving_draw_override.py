"""Unit tests for V4.3 Accuracy-Preserving Selective Draw Override Model.

Verifies:
1. Preset configs (Accuracy Maximizer, Balanced Preserver, Conservative).
...
from models.v4_3_accuracy_preserving_draw_override import (
    V43DrawOverrideConfig,
    V43OverridePrediction,
    evaluate_selective_override,
    predict_v4_3_selective_override,
)
...
def test_preset_configurations():
    """Verify default parameters of V4.3 preset configurations."""
    c_acc = V43DrawOverrideConfig.accuracy_maximizer()
    assert c_acc.name == "accuracy_maximizer"
    assert c_acc.v4_winner_conf_cap == 0.45
    assert c_acc.tot_expected_goals_cap == 2.50
    assert c_acc.abs_elo_cap == 100.0

    c_bal = V43DrawOverrideConfig.balanced_draw_preserver()
    assert c_bal.abs_elo_cap == 150.0

    c_cons = V43DrawOverrideConfig.high_precision_conservative()
...
def test_override_trigger_on_fragile_balanced_match():
    ...
    cfg = V43DrawOverrideConfig.accuracy_maximizer()
    ...
    pred = predict_v4_3_selective_override(
        lambda_home=1.1,
        lambda_away=1.0,
        p_v4=p_v4,
        p_v42=p_v42,
    ...
    assert pred.v4_3_final_decision == "D"
...
def test_override_blocked_on_high_confidence_v4():
    ...
    assert pred.v4_base_decision == "H"
    assert pred.v4_3_final_decision == "H"
