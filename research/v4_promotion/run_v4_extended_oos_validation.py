"""V4 fresh-extended-300 OOS validation — V2 vs V3 vs V4 vs Pinnacle market.

Usage:
    cd "E:\\Football Prediction Project"
    $env:PYTHONPATH="$PWD\\src"
    python research/v4_promotion/run_v4_extended_oos_validation.py

Run test_v4_extended_oos_validation.py FIRST — it must report 0 failures.
...
The 300 fixtures are read from the frozen fresh_extended_fixture_ids.json.
...
PROMOTION RULE (verbatim, enforced in the verdict logic below):
  - do not promote merely because V4 beats V2
  - treat a bootstrap CI crossing zero as NOT statistically distinguishable
...
RESULTS_100 = HERE / "v4_fresh_100_validation_results.json"
...
RESULTS_JSON = HERE / "v4_extended_300_validation_results.json"
REPORT_MD = HERE / "v4_extended_300_validation_report.md"
MANIFEST_JSON = HERE / "v4_extended_300_validation_manifest.json"
...
CLASS_ORDER = ("H", "D", "A")
E6_LR = 0.02
MATERIAL = 0.01            # same threshold as the 50- and 100-match harnesses
BOOTSTRAP_N = 10000
...
def main() -> int:
    print("=" * 78)
    print("V4 FRESH-EXTENDED-300 OOS VALIDATION")
    print("V2 vs V3 vs V4 vs Pinnacle market (reference only)")
    print("=" * 78)

    # ---- PHASE 1 ---------------------------------------------------------
    pre = audit("PHASE 1 — protected integrity (PRE)")
    frozen_pre = {f: md5(HERE / f) for f in FROZEN_PRIOR if (HERE / f).exists()}
    prior_md_pre = {f: md5(HERE / f) for f in
    ...
    # ---- PHASE 2 ---------------------------------------------------------
    print("\n" + "=" * 78)
    print(f"PHASE 2 — FROZEN {N} FIXTURES (read, never regenerated)")
    print("=" * 78)
    ids = [int(x) for x in json.loads(
        FROZEN_IDS.read_text(encoding="utf-8"))["fixture_ids"]]
    if len(ids) != N or len(set(ids)) != N:
        stop(f"frozen list is not {N} unique ids")
    ids_100 = [int(x) for x in json.loads(
    ...
    if len(df) != N: stop("not all frozen ids exist in matches.db")
    ...
    if set(ids) & set(ids_100): stop("overlap with the fresh 100")

    y = np.where(df.home_goals > df.away_goals, "H",
                 np.where(df.home_goals == df.away_goals, "D", "A"))
    comp = dict(Counter(df.competition_name))
    print(f"  {N} fixtures, 0 duplicates, all FT, chronological")
    print(f"  overlap with frozen 20 / frozen 50 / fresh 100: 0 / 0 / 0")
    ...
    # ---- PHASE 3 ---------------------------------------------------------
    print("\n" + "=" * 78)
    print("PHASE 3 — EXTENDED MARKET REFERENCE")
    print("=" * 78)
    ...
    print("=" * 78)
    print("  property: rewrite every 2025/26 outcome AT OR AFTER the target's")
    print("  kickoff; the target and all strictly-earlier frozen fixtures must")
    print("  be bit-identical. (The blanket whole-sample rewrite is NOT used.)")
    ...
    for k in CUTPOINTS:
        ...
        cut_detail.append({"cutpoint": k, "fixture_id": int(tgt),
                           "rewritten": int(m.sum()),
                           "protected": len(earlier), "max_diff": d})
    ...
    def run(art, name):
        E = art.preprocessor.transform(XS[list(art.feature_columns)])
        ...
        if P.shape != (N, 3): stop(f"{name}: wrong shape {P.shape}")
        print(f"  {name}: {N} predictions, mean lambda "
    ...
    # ---- PHASE 8 ---------------------------------------------------------
    print("\n" + "=" * 78)
    print("PHASE 8 — PER-MATCH RECORD")
    print("=" * 78)
    ...
    disagree = int(np.sum([len({p2[i], p3[i], p4[i], pm[i]}) > 1
                           for i in range(N)]))
    print(f"  {N} per-match records captured (full detail in the results JSON)")
    print(f"  fixtures where the four arms do not fully agree: {disagree}/{N}")
    print(f"\n  {'#':>4} {'league':<15} {'match':<40} {'act':>4}"
    ...
    # ---- PHASE 9 ---------------------------------------------------------
    print("\n" + "=" * 78)
    print(f"PHASE 9 — {N}-MATCH SCORECARD")
    print("=" * 78)
    ...
    print(f"\n  {'MODEL':<8}{'CORRECT':>11}{'ACC':>9}{'LOGLOSS':>10}{'BRIER':>9}"
          f"{'RPS':>9}{'DRAW_REC':>10}{'ECE':>9}")
    print("  " + "-" * 75)
    for k in ("V2", "V3", "V4", "MARKET"):
        m = M[k]
        print(f"  {k:<8}{m['correct']:>7}/{N}{m['accuracy']:>9.4f}"
    ...
    for o, Po, po in (("V2", P2, p2), ("V3", P3, p3), ("MARKET", PM, pm)):
        ...
        print(f"    top-class differences: {d['top_class_changes']}/{N}")

    # ---- PHASE 11 --------------------------------------------------------
    print("\n" + "=" * 78)
    print(f"PHASE 11 — CHRONOLOGICAL BUCKETS ({N // BUCKET_SIZE} x {BUCKET_SIZE})")
    print("=" * 78)
    buckets = {f"{i * BUCKET_SIZE + 1}-{(i + 1) * BUCKET_SIZE}":
               slice(i * BUCKET_SIZE, (i + 1) * BUCKET_SIZE)
               for i in range(N // BUCKET_SIZE)}
    bres = {}
    print(f"  {'bucket':<10}{'n':>4}" + "".join(
    ...
    for nm, sl in buckets.items():
        ...
        bres[nm] = e
        print(f"  {nm:<10}{len(yy):>4}" + "".join(
            f"{e[k]['correct']:>6}/{len(yy):<3}{e[k]['log_loss']:>8.4f}"
            for k in ("V2", "V3", "V4", "MARKET")))
    v4_ll_by_bucket = [bres[nm]["V4"]["log_loss"] for nm in buckets]
    print(f"\n  V4 log-loss spread across buckets: "
          f"{min(v4_ll_by_bucket):.4f} .. {max(v4_ll_by_bucket):.4f} "
          f"(range {max(v4_ll_by_bucket) - min(v4_ll_by_bucket):.4f})")
    print("  Bucket-level differences are DESCRIPTIVE; n=50 per bucket is far")
    print("  too small to support a claim about drift.")
    ...
    print(f"\n  Every league has n>={min(comp.values())} — the best per-league")
    print("  coverage in the programme so far. Still DESCRIPTIVE ONLY: no")
    print("  per-league conclusion is drawn, and no per-league selection is")
    print("  made on the basis of these numbers.")

    # ---- PHASE 13 --------------------------------------------------------
    print("\n" + "=" * 78)
    print("PHASE 13 — LONGITUDINAL: 50 vs 100 vs 300")
    print("=" * 78)
    hist_cmp = {"extended_300": {k: M[k] for k in ("V2", "V3", "V4", "MARKET")}}
    for tag, path in (("fifty", RESULTS_50), ("hundred", RESULTS_100)):
        if path.exists():
            d = json.loads(path.read_text(encoding="utf-8"))
            sc = d.get("scorecard") or d.get("metrics") or d.get("summary") or {}
            hist_cmp[tag] = sc
    print(f"  {'model':<8}{'50 acc':>9}{'50 LL':>10}{'100 acc':>10}{'100 LL':>10}"
          f"{'300 acc':>10}{'300 LL':>10}")
    print("  " + "-" * 67)
    ...
    for k in ("V2", "V3", "V4", "MARKET"):
        ...
        h = hist_cmp.get("hundred", {}).get(k, {})
        t = M[k]
        print(f"  {k:<8}{g(f_,'accuracy'):>9}{g(f_,'log_loss'):>10}"
              f"{g(h,'accuracy'):>10}{g(h,'log_loss'):>10}"
              f"{t['accuracy']:>10.4f}{t['log_loss']:>10.6f}")
    print("\n  The 300-match figures supersede the 50 and 100 as the primary")
    print("  evidence: they are the largest sample and fully disjoint from both.")
    ...
    change = {"mean_abs_prob_change": round(float(dP.mean()), 8),
              ...
              "n_materially_changed": int(np.sum(dP.max(axis=1) > MATERIAL)),
    ...
    # ---- PHASE 16 --------------------------------------------------------
    print("\n" + "=" * 78)
    print("PHASE 16 — DETERMINISM (full re-run)")
    print("=" * 78)
    ...
    PMb = pd.read_sql_query(
        "SELECT fixture_id,p_home,p_draw,p_away FROM fresh_extended_market",
    ...
    # ---- PHASE 17 --------------------------------------------------------
    post = audit("PHASE 17 — protected integrity (POST)")
    frozen_post = {f: md5(HERE / f) for f in frozen_pre}
    frozen_ok = frozen_pre == frozen_post
    prior_md_post = {f: md5(HERE / f) for f in prior_md_pre}
    prior_md_ok = prior_md_pre == prior_md_post
    print(f"\n  frozen 20/50/100 result+manifest artifacts unchanged: {frozen_ok}")
    print(f"  prior reports unchanged                              : {prior_md_ok}")
    if not frozen_ok: stop("frozen 20/50/100 artifacts modified")
    if not prior_md_ok: stop("prior reports modified")

    integrity_ok = all(v["status"] == "identical" for v in post.values())
    market_ok = True

    # ---- verdict ---------------------------------------------------------
    # Promotion rule, applied literally:
    #   - beating V2 alone is NOT sufficient
    #   - a bootstrap CI crossing zero means NOT statistically distinguishable
    b_v3 = boot["V4_minus_V3"]
    b_v2 = boot["V4_minus_V2"]
    b_mkt = boot["V4_minus_MARKET"]
    beats_v3 = (not b_v3["ci_crosses_zero"]) and b_v3["mean_delta"] < 0
    beats_v2 = (not b_v2["ci_crosses_zero"]) and b_v2["mean_delta"] < 0
    evidence_sufficient = bool(beats_v3 and beats_v2)
    promoted = False   # this harness NEVER promotes, whatever the numbers say

    # ---- write outputs ---------------------------------------------------
    now = datetime.now(timezone.utc).isoformat()
    results = {
        "generated_at": now,
        "experiment": "V4 fresh-extended-300 OOS validation",
        "sample_note": ("Fresh extended 300 = the next chronological FT "
                        "fixtures after the fresh 100 (LIMIT 300 OFFSET 150). "
                        "Zero overlap with the frozen 20, frozen 50 or fresh "
                        "100. IDs frozen before evaluation."),
        ...
        "market": {"source": EXT_MARKET_DB.name, "md5": md5(EXT_MARKET_DB),
                   "bookmaker": "Pinnacle", "price": "closing",
                   "coverage": f"{N}/{N}", "role": "REFERENCE ONLY",
                   "in_feature_matrix": False},
        "per_match": rows, "scorecard": M, "pairwise": pair,
        "buckets": bres, "by_league": lg,
        "by_league_note": "DESCRIPTIVE ONLY — no per-league conclusion drawn.",
        "history_50_100_300": hist_cmp,
        "v4_vs_v3_change": change, "bootstrap": boot,
        "reliability": {k: reliability(y, P) for k, P in
                        (("V2", P2), ("V3", P3), ("V4", P4), ("MARKET", PM))},
        "causality": {"cutpoints": CUTPOINTS, "detail": cut_detail,
                      "max_diff": worst, "passed": causal_ok,
                      "property": ("rewrite all 2025/26 outcomes at or after "
                                   "the target kickoff; target and all "
                                   "strictly-earlier frozen fixtures must be "
                                   "bit-identical")},
        "determinism": {**dd, "passed": determinism},
        "integrity": {"pre": pre, "post": post, "passed": integrity_ok,
                      "frozen_prior_unchanged": frozen_ok,
                      "prior_reports_unchanged": prior_md_ok},
        "promotion": {
            "promoted": promoted,
            "v4_beats_v3_significantly": beats_v3,
            "v4_beats_v2_significantly": beats_v2,
            "evidence_sufficient_for_promotion": evidence_sufficient,
            "rule": ("Do not promote merely because V4 beats V2. A bootstrap "
                     "CI crossing zero is NOT statistically distinguishable."),
            "note": "Strictly observational. V4 is NOT promoted."},
    }
    RESULTS_JSON.write_text(json.dumps(results, indent=2, default=str),
                            encoding="utf-8")

    overall = (integrity_ok and frozen_ok and prior_md_ok
               and causal_ok and determinism and market_ok)

    MANIFEST_JSON.write_text(json.dumps({
        "generated_at": now, "fixtures": N,
        "results_file": RESULTS_JSON.name, "report_file": REPORT_MD.name,
        "frozen_ids_md5": md5(FROZEN_IDS),
        "market_db_md5": md5(EXT_MARKET_DB),
        "v4_artifact_md5": md5(V4_ARTIFACT),
        "integrity": integrity_ok, "causality": causal_ok,
        "determinism": determinism, "market_coverage": f"{N}/{N}",
    ...
    def row(k):
        m = M[k]
        return (f"| {k} | {m['correct']}/{N} | {m['accuracy']:.4f} | "
    ...
    lines = [
        ...
        "**V4 is NOT promoted.**", "",
        "## Sample", "",
        f"- Selection: `LIMIT 300 OFFSET 150` on the 2025/26 FT fixtures, "
        f"chronological",
        f"- Frozen before evaluation, MD5 `{md5(FROZEN_IDS)}`",
        f"- **Zero overlap** with the frozen 20, frozen 50 or fresh 100",
    ...
    for o in ("V2", "V3", "MARKET"):
        ...
        lines.append(
            ...
            f"{d['correct_delta']:+d} |")
    lines += ["", "## Chronological Buckets (6 x 50)", "",
    ...
    for nm, e in bres.items():
        lines.append(f"| {nm} | {e['V2']['n']} | " + " | ".join(
            ...
            for k in ("V2", "V3", "V4", "MARKET")) + " |")
    lines += ["", "Bucket-level differences are descriptive; n=50 per bucket "
                  "cannot support a drift claim.", "",
    ...
    lines += ["", "No per-league conclusion is drawn and no per-league "
                  "selection is made from these numbers.", "",
    ...
    for k in ("V2", "V3", "V4", "MARKET"):
        f_ = hist_cmp.get("fifty", {}).get(k, {})
        h = hist_cmp.get("hundred", {}).get(k, {})
        lines.append(f"| {k} | {g(f_,'accuracy')} | {g(f_,'log_loss')} | "
                     f"{g(h,'accuracy')} | {g(h,'log_loss')} | "
                     f"{M[k]['accuracy']:.4f} | {M[k]['log_loss']:.6f} |")
    lines += ["", "## V4 vs V3 Probability Stability", "",
              f"- Mean absolute probability change: "
              f"{change['mean_abs_prob_change']:.8f}",
              f"- Max absolute probability change: "
              f"{change['max_prob_change']:.8f}",
              f"- Materially changed (>{MATERIAL}): "
              f"{change['n_materially_changed']}/{N} "
              f"({change['pct_materially_changed']:.2f}%)",
              f"- Top-class changes: {change['top_class_changes']}/{N}", "",
    ...
    lines += ["", "## Gates", "",
              ...
              f"- MARKET COVERAGE: PASS ({N}/{N})",
              f"- **OVERALL: {'COMPLETE' if overall else 'BLOCKED'}**",
              "", "## Promotion Decision", "",
              "Rule applied: do not promote merely because V4 beats V2; treat "
              "a bootstrap CI crossing zero as NOT statistically "
              "distinguishable.", "",
              f"- V4 significantly better than V3: **{beats_v3}**",
              f"- V4 significantly better than V2: **{beats_v2}**",
              f"- Evidence sufficient for promotion: "
              f"**{evidence_sufficient}**", ""]
    if b_v3["ci_crosses_zero"]:
        lines.append("- V4 vs V3 is NOT STATISTICALLY DISTINGUISHABLE.")
    if b_mkt["ci_crosses_zero"]:
        lines.append("- V4 vs MARKET is NOT STATISTICALLY DISTINGUISHABLE.")
    lines.append("")
    if evidence_sufficient:
        lines.append("**PROMOTION CANDIDATE — APPROVAL REQUIRED**")
        lines.append("")
        lines.append("V4 showed statistically distinguishable improvement over V3 and V2 on the frozen 300 OOS sample. Manual approval required before any production changes.")
    else:
        lines.append("**V4 IS NOT PROMOTED. No production file was modified.**")
    REPORT_MD.write_text("\n".join(lines), encoding="utf-8")

    # ---- final summary ---------------------------------------------------
    print("\n" + "=" * 78)
    print(f"V4 FRESH-EXTENDED-{N} OOS VALIDATION")
    print("=" * 78)
    for k in ("V2", "V3", "V4", "MARKET"):
        print(f"\n{k}:")
        print(f"  correct  = {M[k]['correct']}/{N}")
        print(f"  accuracy = {M[k]['accuracy']:.4f}")
        print(f"  log loss = {M[k]['log_loss']:.6f}")
    for o in ("V2", "V3", "MARKET"):
        d = pair[f"V4_vs_{o}"]
        print(f"\nV4 vs {o}:")
        ...
        print(f"  Correct {d['correct_delta']:+d}  "
              f"Accuracy {d['accuracy_delta']:+.4f}")
    print("\n50 vs 100 vs 300 (accuracy / log loss):")
    for k in ("V2", "V3", "V4", "MARKET"):
        f_ = hist_cmp.get("fifty", {}).get(k, {})
        h = hist_cmp.get("hundred", {}).get(k, {})
        print(f"  {k:<7} 50: {g(f_,'accuracy'):>7}/{g(f_,'log_loss'):>8}   "
              f"100: {g(h,'accuracy'):>7}/{g(h,'log_loss'):>8}   "
              f"300: {M[k]['accuracy']:.4f}/{M[k]['log_loss']:.6f}")
    ...
    print("\nPROMOTION DECISION")
    print(f"  V4 significantly better than V3: {beats_v3}")
    print(f"  V4 significantly better than V2: {beats_v2}")
    print(f"  evidence sufficient for promotion: {evidence_sufficient}")
    if b_v3["ci_crosses_zero"]:
        print("  V4 vs V3 is NOT STATISTICALLY DISTINGUISHABLE.")
    if b_mkt["ci_crosses_zero"]:
        print("  V4 vs MARKET is NOT STATISTICALLY DISTINGUISHABLE.")
    print("  (beating V2 alone is not sufficient; a CI crossing zero means")
    print("   the two are NOT statistically distinguishable)")
    if evidence_sufficient:
        print("\nPROMOTION CANDIDATE — APPROVAL REQUIRED")
    else:
        print("\nV4 IS NOT PROMOTED. No production file was modified.")
