"""Unit tests for V4.6 Physical Draw Gate Model.

Verifies:
1. Preset configs (Robust Optimal, High-Precision, Strict Parity).
2. Physical gate constraints (Confidence cap, total goals expectancy, Elo parity, low-score floor).
3. Simplex probability conservation.
4. Deterministic non-mutating execution.
...
def test_preset_configurations():
    """Verify default parameters of V4.6 preset configurations."""
    c_rob = V46PhysicalGateConfig.robust_optimal_gate()
    assert c_rob.name == "robust_optimal_gate"
    assert c_rob.v4_winner_conf_cap == 0.45
    assert c_rob.tot_expected_goals_cap == 2.50
    assert c_rob.abs_elo_cap == 100.0

    c_hp = V46PhysicalGateConfig.high_precision_gate()
    assert c_hp.abs_elo_cap == 80.0

    c_strict = V46PhysicalGateConfig.strict_parity_gate()
    assert c_strict.low_score_prob_floor == 0.24
...
def test_override_trigger_on_fragile_balanced_match():
    ...
    pred = predict_v4_6_physical_gate(
        lambda_home=1.1,
        lambda_away=1.0,
        p_v4=p_v4,
        p_v42=p_v42,
...
def test_override_blocked_on_high_confidence_v4():
    ...
    assert pred.v4_base_decision == "H"
    assert pred.v4_6_final_decision == "H"
