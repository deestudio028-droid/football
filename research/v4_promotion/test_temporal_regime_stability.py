r"""Pre-evaluation behavioural test suite for Phase 6 Temporal / Recency & Regime Stability Research.

Usage:
    $env:PYTHONPATH="$PWD\src;$PWD\research\dixon_coles"
    python research/v4_promotion/test_temporal_regime_stability.py

MUST pass with 0 failures before running temporal_regime_stability_experiment.py.
"""
...
# --- Suite 2: DC+Elo Hybrid Reconstruction Math ---
def suite2() -> Suite:
    s = Suite("2. DC+Elo Hybrid Reconstruction Math")
    compl = json.loads(COMPL_FROZEN_METHOD.read_text(encoding="utf-8"))
    s.check("primary_hybrid" in compl, "Complementarity frozen method contains primary hybrid")
    
    inter = compl["primary_hybrid"]["intercept"]
    w = compl["primary_hybrid"]["weights"]
    s.check(len(w) == 2, "Hybrid weights has dimension 2 (DC, Elo)", f"len={len(w)}")

    # Test sample probability stacking
    p_d_dc = 0.260
    p_d_elo = 0.255
    z_dc = np.log(p_d_dc / (1.0 - p_d_dc))
    z_elo = np.log(p_d_elo / (1.0 - p_d_elo))
    z_comb = inter + w[0] * z_dc + w[1] * z_elo
    p_d_comb = 1.0 / (1.0 + np.exp(-z_comb))
    s.check(0.20 < p_d_comb < 0.35, "Stacked draw probability in reasonable domain", f"p_d={p_d_comb:.4f}")
    return s


# --- Suite 3: Recency Exponential Decay Weights ---
def suite3() -> Suite:
    s = Suite("3. Recency Exponential Decay Weights")
    ages = np.array([0.0, 1.0, 2.0, 3.0])  # in seasons
    hl = 2.0
    weights = np.exp(-np.log(2.0) * ages / hl)
    s.check(abs(weights[0] - 1.0) < 1e-12, "Age 0 weight = 1.0", f"w0={weights[0]}")
    s.check(abs(weights[2] - 0.5) < 1e-12, "Age equal to half-life weight = 0.5", f"w2={weights[2]}")
    s.check(bool(np.all(np.diff(weights) < 0)), "Decay weights strictly monotonically decreasing")
    return s
...
def main() -> int:
    print("=" * 78)
    print("PHASE 6 TEMPORAL / REGIME STABILITY — PRE-EVALUATION TEST SUITE")
    print("=" * 78)
    ...
    if tf > 0:
        print("\n  STOP — do not run temporal_regime_stability_experiment.py.")
