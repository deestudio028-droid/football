"""V4 fresh-100 OOS validation — pre-evaluation behavioural test suite.

Usage:
    cd "E:\\Football Prediction Project"
    $env:PYTHONPATH="$PWD\\src"
    python research/v4_promotion/test_v4_fresh_100_validation.py

MUST pass with 0 failures before run_v4_fresh_100_validation.py.

Per Phase 18 every assertion is BEHAVIOURAL against real data. No string
searching, no assertions about comments or prose.

This file never writes to, or modifies, the frozen 20/50 artifacts, the
frozen 100 fixture list, or the fresh market database.
"""
...
OLD_MARKET_DB = HERE / "promotion_market_odds.sqlite"
...
def suite1() -> Suite:
    s = Suite("1. Frozen 100 Fixture Set")
    s.check(FROZEN_IDS.exists(), "Frozen fixture file exists")
    s.check(md5(FROZEN_IDS) == PINNED[
        "research/v4_promotion/fresh_100_fixture_ids.json"],
        "Frozen fixture file MD5 unchanged", md5(FROZEN_IDS))

    ids = frozen_ids()
    s.check(len(ids) == 100, "Exactly 100 fixture IDs", f"n={len(ids)}")
    s.check(len(set(ids)) == 100, "Zero duplicates")
    ...
    s.check(len(df) == 100, "All 100 exist in matches.db", f"{len(df)}/100")
    ...
    s.check(sum(comp.values()) == 100, "Composition sums to 100",
            " · ".join(f"{k} {v}" for k, v in
                       sorted(comp.items(), key=lambda kv: -kv[1])))
    s.check(len(comp) == 5, "All five target leagues represented",
            f"{len(comp)} leagues")
    s.check(min(comp.values()) >= 15,
            "Every league has n>=15 (better coverage than the 50-match set)",
...
def suite3() -> Suite:
    ...
    s.check(all(i in elo.index for i in ids), "Elo available 100/100")
    ...
    s.check(all(i in st.index for i in ids), "A/D available 100/100")
    ...
    n = conn.execute("SELECT COUNT(*) FROM feature_rows WHERE fixture_id IN "
                     f"({','.join('?' * 100)})", ids).fetchone()[0]
    conn.close()
    s.check(n == 100, "Feature rows exist 100/100", f"{n}/100")
    return s
...
def suite4() -> Suite:
    s = Suite("4. Multi-Cutoff Adversarial Causality")
    ...
    s.check(int(dup.max()) > 1,
            "Fresh 100 contains simultaneous kickoffs (two-pass load-bearing)",
...
def suite5() -> Suite:
    s = Suite("5. Fresh Market Reference (100/100, isolated)")
    s.check(FRESH_MARKET_DB.exists(), "Fresh market DB exists")
    s.check(md5(FRESH_MARKET_DB) == PINNED[
        "research/v4_promotion/fresh_100_market_odds.sqlite"],
        "Fresh market DB MD5 unchanged", md5(FRESH_MARKET_DB))
    ...
    mk = pd.read_sql_query(
        """SELECT fixture_id, p_home, p_draw, p_away, closing_home,
           ...
           FROM fresh_100_market""", conn)
    cols = [r[1] for r in conn.execute("PRAGMA table_info(fresh_100_market)")]
    conn.close()

    s.check(len(mk) == 100, "Exactly 100 rows", f"n={len(mk)}")
    ...
    # the fresh DB must be disjoint from the old 50 DB
    conn = sqlite3.connect(f"file:{OLD_MARKET_DB}?mode=ro", uri=True)
    old = set(r[0] for r in conn.execute("SELECT fixture_id FROM promotion_market"))
    conn.close()
    s.check(not (set(mk.fixture_id) & old),
            "Fresh market DB is disjoint from the 50-match market DB",
            f"old={len(old)} rows, overlap=0")
    return s
...
def suite6() -> Suite:
    ...
    for nm, path, loader in (("V2", V2_ARTIFACT, load_v2),
        ...
        a = loader(path)
        E = a.preprocessor.transform(X100[list(a.feature_columns)])
        ...
        s.check(P.shape == (100, 3), f"{nm} produced 100 predictions",
                f"{P.shape}")
...
def suite7() -> Suite:
    ...
    m1 = pd.read_sql_query("SELECT fixture_id,p_home,p_draw,p_away "
                           "FROM fresh_100_market ORDER BY fixture_id", conn)
...
def suite8() -> Suite:
    ...
    # Load-bearing artifacts: Phase 13 reads the RESULTS JSON, so those and
    # the manifests are required. The 50-match .md was declared in that
    # harness but never written -- a known cosmetic gap in the reporting
    # step, with the data itself intact in the JSON. Asserted separately so
    # a missing report cannot masquerade as missing results.
    ...
    # the frozen results must be readable and carry a usable scorecard,
    # which is what Phase 13 actually depends on
    for f in ("v4_20_validation_results.json", "v4_50_validation_results.json"):
        p = HERE / f
        if not p.exists():
        ...
        try:
            ...
            s.check(has, f"{f} carries a scorecard usable by Phase 13",
                    f"top-level keys: {len(d)}")
        except Exception as exc:                              # noqa: BLE001
            s.bad(f"{f} parses as JSON", str(exc))

    s.check(OLD_MARKET_DB.exists(), "50-match market DB present")
    return s


def main() -> int:
    print("=" * 78)
    print("V4 FRESH-100 OOS VALIDATION — PRE-EVALUATION TEST SUITE")
    print("=" * 78)
    ...
    if tf:
        print("\n  STOP — do not run run_v4_fresh_100_validation.py.")
