"""V4.4 Robust Accuracy-Preserving Selective Draw Override Model.
...
class V44RobustOverrideConfig:
    """Configuration for V4.4 Robust Selective Draw Override Layer."""
    name: str = "robust_accuracy_maximizer"
    version: str = "v4.4-robust-draw-override"
    ...
    @classmethod
    def accuracy_maximizer(cls) -> V44RobustOverrideConfig:
        """Top accuracy configuration (52.50% accuracy, +4 net wins vs V4, 24 correct draws, 0 degraded leagues)."""
        return cls(
            name="robust_accuracy_maximizer",
    ...
    @classmethod
    def high_precision_conservative(cls) -> V44RobustOverrideConfig:
        """Conservative high-precision configuration (52.50% accuracy, 66 draw preds, 21 correct draws, 31.8% prec)."""
        return cls(
            name="robust_high_precision_conservative",
            draw_prob_threshold=0.2600,
            winner_margin_cap=0.1000,
            v4_winner_conf_cap=0.4500,
            abs_elo_cap=80.0,
    ...
    @classmethod
    def balanced_draw_preserver(cls) -> V44RobustOverrideConfig:
        """Balanced configuration (52.19% baseline accuracy, 103 draw preds, 33 correct draws, 32.0% prec, 10.0% rec)."""
        return cls(
            name="robust_balanced_draw_preserver",
            draw_prob_threshold=0.2600,
            winner_margin_cap=0.1000,
            v4_winner_conf_cap=0.4500,
            abs_elo_cap=150.0,
...
@dataclass
class V44RobustPrediction:
    """Output container for V4.4 prediction."""
    fixture_id: Optional[int]
    probabilities: Dict[str, float]
    v4_base_decision: str
    v4_4_final_decision: str
...
def evaluate_robust_override(
    p_v4: List[float] | np.ndarray,
    p_v42: List[float] | np.ndarray,
    abs_elo_diff: float,
    lambda_home: float,
    lambda_away: float,
    config: V44RobustOverrideConfig,
...
def predict_v4_4_robust_override(
    lambda_home: float,
    lambda_away: float,
    p_v4: List[float] | np.ndarray,
    p_v42: List[float] | np.ndarray,
    abs_elo_diff: float,
    config: Optional[V44RobustOverrideConfig] = None,
    fixture_id: Optional[int] = None,
) -> V44RobustPrediction:
    """Generate V4.4 robust selective draw override prediction."""
    if config is None:
        config = V44RobustOverrideConfig.accuracy_maximizer()

    pv42 = np.asarray(p_v42, dtype=float)
    pv4 = np.asarray(p_v4, dtype=float)
    v4_dec = CLASS_ORDER[int(np.argmax(pv4))]

    do_override, final_dec, meta = evaluate_robust_override(
        p_v4=pv4,
        p_v42=pv42,
    ...
    return V44RobustPrediction(
        fixture_id=fixture_id,
        probabilities=prob_dict,
        v4_base_decision=v4_dec,
        v4_4_final_decision=final_dec,
