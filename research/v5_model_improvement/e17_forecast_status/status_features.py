"""Phase 42 — Experiment E17: Causal Pre-Match Features for Forecast Status Classification.

Extracts strictly pre-kickoff causal signals:
- E10 probability geometry (normalized entropy, max probability, margins, concentration)
- Poisson intensities and scoring ratios
- Elo parity indicators
- Season timeline and team sample maturity counters
...
def extract_status_features(
    ...
    """Extracts strictly causal pre-match features for decision status classification."""
    ...
    prob_concentration = np.sum(p ** 2, axis=1)  # Herfindahl index
