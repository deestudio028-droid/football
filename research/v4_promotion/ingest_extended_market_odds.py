"""Extended fresh OOS market odds ingestion — Phase 8 blocker resolution.

Usage:
    cd "E:\\Football Prediction Project"
    $env:PYTHONPATH="$PWD\\src"
    python research/v4_promotion/ingest_fresh_extended_market_odds.py
...
-----------------------
    research/v4_promotion/fresh_extended_market_odds.sqlite   <- NEW, isolated
...
OUT_DB = HERE / "fresh_extended_market_odds.sqlite"
MANIFEST = HERE / "fresh_extended_market_ingest_manifest.json"
...
def load_frozen() -> tuple[list[int], dict]:
    ...
    if len(ids) != 300:
        blocked(f"Frozen list holds {len(ids)} ids, expected 300")
    if len(set(ids)) != 300:
...
def main() -> int:
    print("=" * 78)
    print("FRESH EXTENDED MARKET ODDS INGEST — Pinnacle closing 1X2, power de-vig")
    print("=" * 78)
    ...
    for rel in ("research/v4_promotion/fresh_100_market_odds.sqlite",
        ...
        print(f"  [{'OK  ' if ok else 'FAIL'}] {rel}")
        if not ok:
            blocked(f"prior market database changed: {rel}")
    ...
    print(f"  exactly 300          : {len(ids) == 300}")
    print(f"  zero duplicates      : {len(set(ids)) == 300}")
    ...
    prior = set()
    for db, tbl in ((old_market, "promotion_market"),
                    (HERE / "fresh_100_market_odds.sqlite", "fresh_100_market")):
        if db.exists():
            c = sqlite3.connect(f"file:{db}?mode=ro", uri=True)
            prior |= set(r[0] for r in c.execute(f"SELECT fixture_id FROM {tbl}"))
            c.close()
    overlap = set(ids) & prior
    print(f"  overlap with prior   : {len(overlap)} (across {len(prior)} prior rows)")
    if overlap:
        blocked(f"frozen 300 overlaps prior market data: {sorted(overlap)[:10]}")
    ...
    conn.execute("""
        CREATE TABLE IF NOT EXISTS fresh_extended_market (
    ...
    for seq, fid in enumerate(ids, 1):
        ...
        conn.execute(
            "INSERT OR REPLACE INTO fresh_extended_market VALUES "
            "(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)",
    ...
    if failures:
        ...
        conn.close()
        blocked(f"{len(failures)}/300 fixtures could not be ingested. "
    ...
    rows = conn.execute(
        ...
        "closing_home, closing_draw, closing_away FROM fresh_extended_market"
    ).fetchall()
    cols = [r[1] for r in conn.execute("PRAGMA table_info(fresh_extended_market)")]

    coverage = len(rows) == 300 and {r[0] for r in rows} == set(ids)
    ...
    print(f"  coverage 300/300        : {coverage} ({len(rows)} rows)")
    ...
    for k, v in [("data_class", DATA_CLASS), ("bookmaker", "Pinnacle"),
                 ...
                 ("purpose", "extended fresh OOS validation reference (300)"),
    ...
    MANIFEST.write_text(json.dumps({
        "generated_at": now,
        "purpose": "Extended fresh OOS market reference (300 fixtures)",
        ...
        "validation": {"coverage_300_of_300": coverage,
    ...
    print("\n" + "=" * 78)
    print("FRESH EXTENDED MARKET ODDS INGEST")
    print("=" * 78)
    print(f"FIXTURES: 300")
    print(f"SUCCESS: {ok_n}/300")
    print(f"FAILED: {len(failures)}/300")
    ...
    print(f"PRIOR MARKET DBS UNCHANGED: {mark(old_unchanged)}")
    print(f"PROTECTED FILES UNCHANGED: {mark(protected_ok)}")
    print(f"OVERALL: {'PASS' if overall else 'BLOCKED'}")

    if overall:
        print("\nExtended market reference ready. Harness NOT built,")
        print("predictions NOT run.")
